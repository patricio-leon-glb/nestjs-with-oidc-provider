...
export class OidcController {
  ...
  public async All(@Req() req: Request, @Res() res: Response) {
    const { params, query, body, originalUrl } = req;
  ...
  // @Get('/auth')
  // public async root(@Req() req, @Res() res) {
  //   return this.oidcCallback(req, res);
  // }
  ...
  public async interactionLogin(@Req() req, @Res() res) {
    try {
      this.logger.log(`POST interaction/${req.params.uid}/login`);
      ...
      const account = {
        ...
        name: 'patricioleon@gmail.com',
      }; //await Account.findByLogin({});
  ...
  public async interactionConfirm(
    @Req() req,
    @Res() res,
    @Param('uid') uid: string,
  ) {
    try {
      this.logger.log(`POST interaction/${uid}/confirm`);
